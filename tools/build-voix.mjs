...
const PHRASE_ESSAI = 'Ce soir, un tout fin croissant, comme un sourire dans le ciel… '
  + 'La Lune ne change pas vraiment de forme : c’est la lumière du Soleil qui joue à cache-cache avec nous !';
...
let refaire = {};
try { refaire = JSON.parse(localStorage.getItem('lune-ecoute-refaire') || '{}'); } catch (e) {}
let repris = -1;
if (!filtre) {
  try { repris = parseInt(localStorage.getItem('lune-ecoute-position') || '-1', 10); } catch (e) {}
}
...
function sauve() {
  try {
    localStorage.setItem('lune-ecoute-refaire', JSON.stringify(refaire));
    // en écoute ciblée, ne pas écraser la position de l'écoute complète
    if (!filtre) localStorage.setItem('lune-ecoute-position', String(courant));
  } catch (e) {}
}
function marque(i) {
  const id = DATA[i].id;
...
function contexteDe(b) {
  ...
  if (b.id.indexOf('scn-') === 0) return 'Regarde la Lune, ce soir…';
  if (/^defi-.*-consigne$/.test(b.id)) return 'On joue à attraper la bonne Lune !';
  if (/^defi-.*-bravo$/.test(b.id)) {
    // le bravo répond à sa consigne : elle ancre le ton de la victoire
    const consigne = blocs.find((x) => x.id === b.id.replace(/-bravo$/, '-consigne'));
    return consigne ? consigne.texte : null;
  }
  return null;
}
